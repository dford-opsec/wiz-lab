...
This environment was purpose-built for the Wiz Technical Exercise. It follows modern DevOps principles but has been engineered with intentional configuration weaknesses and vulnerabilities 
to simulate real-world threat vectors and demonstrate cloud native security tooling.

Architecture

The environment consists of a public-facing GCP Cloud Load Balancer routing traffic to a Go-based web application running on a private Google Kubernetes Engine (GKE) cluster. The application 
connects to a MongoDB database hosted on a standalone Compute Engine VM in a public subnet.
...
flowchart LR
    ...
    classDef subnet fill:none,stroke:#9aa0a6,stroke-width:2px,stroke-dasharray:5
...
IaC Deployment (terraform.yml): Automatically provisions the foundational GCP infrastructure (VPC, Subnets, GKE Cluster, Compute Engine VM, Storage Bucket, and Firewall rules). 
